  ...
  <div class="modal modal-blur fade" id="modal-edit" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        <form action="<?= base_url("data/add_inventory_item") ?>" method="POST" enctype="multipart/form-data">
          ...
          <div class="modal-body">
            ...
            <div class="mb-1">
              ...
              <input type="text" class="form-control" name="inventory_item_name" placeholder="Inventory Item name" required>
            </div>
            <div class="mb-1">
              <div class="form-label">Inventory Type</div>
            ...
            <div class="mb-1">
              ...
              <select class="form-select" name="id_unit" required>
            ...
            <div class="mb-1">
              ...
              <textarea type="text" class="form-control" name="description" placeholder="Inventory Item description"></textarea>
            </div>
          </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    $("select[name=inventory_type]").on("change", function() {
      var id = $(this).val();
      if(id == ""){
        id = 0;
      }
      $.ajax({
        url: "<?= base_url('data/get_category_item_filter/') ?>"+id,
        cache: false,
        type: "GET",
        success: function(response){
          $("select[name=id_category]").html(response);
        },
        error: function(xhr) {
    ...
    var table = $('#mytable').DataTable({
      ...
      "ajax": {
        ...
        "url": "<?= base_url('data/load_inventory_item_dt') ?>",
        "dataType": "json",
        "type": "POST",
      },
      columns: [
    ...
    $("#modal-edit").on('show.bs.modal', function(e) {
      $('input[name=id]').val("");
      $('input[name=inventory_item_name]').val("");
      ...
      if(btn.data("mode") == "Edit"){
        ...
        $.ajax({
          url: "<?= base_url('data/load_inventory_item/') ?>"+btn.data("id"),
          cache: false,
          type: "GET",
          success: function(response) {
            $('input[name=id]').val(response.id_inventory_item);
            $('input[name=inventory_item_name]').val(response.inventory_item_name);
            $('select[name=inventory_type]').val(response.inventory_type);
            $('textarea[name=description]').val(response.description);
            $('select[name=id_category]').val(response.id_category);
            $('select[name=id_unit]').val(response.id_unit);
          },
          error: function(xhr) {
            alert("Failed to load data!");
          }
    ...
    $(document).on('click', '.btn-delete', function(){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "<?= base_url("data/delete_inventory_item/") ?>"+id;
        }
      });
    });
