  ...
  <div class="modal modal-blur fade" id="modal-edit" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
      <div class="modal-content">
        <form action="<?= base_url("data/add_unit") ?>" method="POST" enctype="multipart/form-data">
  ...
  $(document).ready(function(){
    var table = $('#mytable').DataTable({
      ...
      "ajax": {
        ...
        "url": "<?= base_url('data/load_unit_dt') ?>",
        "dataType": "json",
        "type": "POST",
      },
      columns: [
    ...
    $("#modal-edit").on('show.bs.modal', function(e) {
      ...
      if(btn.data("mode") == "Edit"){
        ...
        $.ajax({
          url: "<?= base_url('data/load_unit/') ?>"+btn.data("id"),
          cache: false,
          type: "GET",
          success: function(response) {

            $('input[name=id]').val(response.id_unit);
            $('input[name=unit_name]').val(response.unit_name);
    ...
    $(document).on('click', '.btn-delete', function(){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "<?= base_url("data/delete_unit/") ?>"+id;
        }
      });
    });
