  ...
  $(document).ready(function(){
    ...
    var table = $('#mytable').DataTable({
      ...
      "ajax": {
        ...
        "url": "<?= base_url('bmn/load_stock_in_dt') ?>",
        "dataType": "json",
        "type": "POST",
        data: function(d){
    ...
    $("#modal-edit").on('show.bs.modal', function(e) {
      ...
      $("#serial-number-div").html('<input type="text" class="form-control serial-number" name="serial_number[0]" placeholder="Serial Number 1" required>');
      ...
      if(btn.data("mode") == "Edit"){
        ...
        $.ajax({
          url: "<?= base_url('bmn/load_stock_in/') ?>"+btn.data("id"),
          cache: false,
          type: "GET",
          success: function(response) {
            ...
            $.ajax({
              url: "<?= base_url('bmn/load_stock/') ?>"+response.id_stock_in,
              cache: false,
              type: "GET",
                ...
                response2.forEach(
                  function myFunction(item, index) {
                    if(item.img == ""){
                      ...
                      $('#imgPreview').attr('src', '<?= base_url("public/bmn/") ?>'+item.img);
                    }
                    $("#serial-number-div").append('<input type="text" class="form-control serial-number" name="serial_number['+index+']" placeholder="Serial Number '+(index+1)+'" value="'+item.serial_number+'" required>');
                  });
              }
            });
          },
    ...
    $(document).on('click', '.btn-delete', function(){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "<?= base_url("bmn/delete_stock_in/") ?>"+id;
        }
      });
    });
