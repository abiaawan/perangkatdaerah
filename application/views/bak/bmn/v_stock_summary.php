...
<div class="section-to-print" id="section-to-print" style="visibility: hidden;">
  <div>
    <div id="qrimg"></div>
    <div id="qrlabel" class="text-center"></div>
  </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $("#export-excel").on("click", function() {
      window.location = "<?= base_url("export/bmn_stock_export/") ?>?inventory=" + $(this).data("id") + "&location=" + $(this).val();;
    });
    ...
    function openAddStockModal(btn){
      $.ajax({
        url: "<?= base_url('bmn/load_stock_row/') ?>"+btn.data("id"),
        cache: false,
        type: "GET",
        success: function(response){
          ...
          if(response.img == ""){
            ...
            $('#imgPreview').attr('src', '<?= base_url("public/bmn/") ?>'+response.img);
          }
      ...
      $.ajax({
        ...
        success: function(response) {
          if(response.img == ""){
            ...
            $('#imgPreviewItem').attr('src', '<?= base_url("public/bmn/") ?>'+response.img);
          }
    ...
    var table = $('#mytable').DataTable({
      ...
      "ajax": {
        ...
        "url": "<?= base_url('bmn/load_stock_summary_dt') ?>",
        "dataType": "json",
        "type": "POST",
        data: function(d){
    ...
    $("#modal-edit").on('show.bs.modal', function(e) {
      ...
      table2 = $('#mytable2').DataTable({
        ...
        "ajax": {
          ...
          "url": "<?= base_url('bmn/load_stock_list_dt/') ?>",
          "dataType": "json",
          "type": "POST",
          "data": function(d) {
            d.id = id;
    ...
    $(document).on("click","#print-qr", function() {
      ...
      var btn = $(this);
      $.ajax({
        url: "<?= base_url('bmn/load_qr/') ?>"+$("input[name=data_export]").val(),
        cache: false,
        type: "GET",
        success: function(response){
          $("#section-to-print").html(response);
          $("#section-to-print").ready(function() {
            window.print();
          });
        },
        error: function(xhr) {
